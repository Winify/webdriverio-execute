    ...
    mockExecute: vi.fn().mockResolvedValue([
      {
        ...
        selector: 'button.submit', isInViewport: true, id: '',
        name: '', className: '', value: '', placeholder: '', href: '',
        ariaLabel: '', role: '', src: '', alt: '',
      },
      {
        tagName: 'input', type: 'email', textContent: '',
        selector: '#email', isInViewport: true, id: 'email',
        name: '', className: '', value: '', placeholder: 'you@example.com',
...
import { handler as fillHandler } from '../src/commands/type';
...
describe('integration: open -> snapshot -> click -> fill -> screenshot -> close', () => {
  ...
  it('should complete the full workflow', async () => {
    ...
    await snapshotHandler({
      ...
      visible: true,
      _sessionsDir: TEST_DIR,
    } as unknown as Parameters<typeof snapshotHandler>[0]);
