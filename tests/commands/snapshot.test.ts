...
describe('snapshot command', () => {
  ...
  it('should capture browser elements and write refs', async () => {
    mockExecute.mockResolvedValue([
      ...
      { tagName: 'input', name: 'Email', type: 'email', value: '', href: '', selector: '#email', isInViewport: true },
    ]);

    await handler({ session: 'default', visible: true, a11y: false, _sessionsDir: TEST_DIR } as unknown as Parameters<typeof handler>[0]);
  ...
  it('should error when no session exists', async () => {
    await handler({ session: 'nonexistent', visible: true, a11y: false, _sessionsDir: TEST_DIR } as unknown as Parameters<typeof handler>[0]);
