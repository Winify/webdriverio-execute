...
describe('click command', () => {
  ...
  beforeEach(async () => {
    ...
    await writeRefs(getRefsPath('default', TEST_DIR), {
      e1: { selector: 'button.submit', tagName: 'button', text: 'Submit' },
      e2: { selector: '#email', tagName: 'input', text: '' },
    });
  ...
  it('should error on unknown ref', async () => {
    await handler({ ref: 'e99', session: 'default', _sessionsDir: TEST_DIR } as unknown as Parameters<typeof handler>[0]);
