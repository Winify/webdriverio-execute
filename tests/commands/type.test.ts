...
describe('type command', () => {
  ...
  beforeEach(async () => {
    ...
    await writeRefs(getRefsPath('default', TEST_DIR), {
      e1: { selector: '#email', tagName: 'input', text: '' },
    });
  ...
  it('should clear and type input by ref', async () => {
    await handler({ ref: 'e1', text: 'hello@example.com', session: 'default', _sessionsDir: TEST_DIR } as unknown as Parameters<typeof handler>[0]);
